...
class KeyboardListener(Node):
    def __init__(self, arm_control, husky_control, gripper_control):
        ...
        self.subscription = self.create_subscription(Int32, '/keyboard/keypress', self.keyboard_callback, 10)
    ...
    def keyboard_callback(self, msg):
        ...
        try: 
            ...
            # Map key characters to specific actionstiit
            if key_char == 'H': # Move the arm to the target HORIZONTAL configuration
                self.arm_control.move_arm_to([0.229, 1.311, 0.118, 2.413, 0.298, -2.100, -1.520], 5)
            elif key_char == 'V': # Move the arm to the target VERTICAL configuration
                self.arm_control.move_arm_to([0.05565855, 0.58595939, 0.01642355, 1.78789783, 0.0384496, 0.77841685, -1.53400479], 5)
                ...
                self.arm_control.move_arm_to([-0.23921483, 0.85088292, 0.1007404, 2.40223628, -0.07318166, -1.70646077, -1.63699667],5)
                #self.arm_control.move_arm_to([0.1, 1.0, 0.118, 2.213, 0.298, -1.900, -1.020], 5)
            elif key_char == 'R': # RESET the robot and grasp box positions
                self.husky_control.reset_poses()
            elif key_char != 'W' and key_char != 'A' and key_char != 'S' and key_char != 'D':
                self.get_logger().info(f"Key {key_char} is not defined for any action.")

        except ValueError:
            # Print a warning message instead of exiting the program
            self.get_logger().error(f"Key code {key_code} cannot be converted to a valid character.")
...
class HuskyRobotControl(Node):
    def __init__(self):
        ...
        self.odom_sub = self.create_subscription(Odometry, '/a200_0000/platform/odom/filtered', self.odom_callback, 10)
        self.current_position = None
        self.target_position = None

    def odom_callback(self, msg):
        """Callback function to store the robot's current position"""
    ...
    def reset_poses(self):
        ...
        try:
            ...
            subprocess.run(['ign', 'service', '-s', '/world/warehouse/set_pose',
                            '--reqtype', 'ignition.msgs.Pose',
                            '--reptype', 'ignition.msgs.Boolean',
                            '--timeout', '5000',
                            '--req', 'position: {x: 0.000925, y: 0.000067, z: 0.132280}, '
            ...
            subprocess.run(['ign', 'service', '-s', '/world/warehouse/set_pose',
                            ...
                            '--reptype', 'ignition.msgs.Boolean',
                            '--timeout', '5000',
                            '--req', 'position: {x: 3.0, y: 0.0, z: 0.53}, '
...
class ArmControl(Node):
    def __init__(self):
        ...
        self.arm_pub = self.create_publisher(JointTrajectory, '/a200_0000/arm_0_joint_trajectory_controller/joint_trajectory', 10)
        # Subscribe to the state topic to monitor the arm trajectory
        self.arm_state_sub = self.create_subscription(JointTrajectoryControllerState, '/a200_0000/arm_0_joint_trajectory_controller/state', self.arm_state_callback, 10)
        self.current_state = None

    def arm_state_callback(self, msg):
        """Callback to store the latest state of the arm"""
    ...
    def move_arm_to(self, positions, duration):
        ...
        traj.joint_names = ['arm_0_joint_1', 'arm_0_joint_2', 'arm_0_joint_3', 'arm_0_joint_4', 'arm_0_joint_5', 'arm_0_joint_6', 'arm_0_joint_7']
...
class GripperControl(Node):
    def __init__(self):
        ...
        self.gripper_client = ActionClient(self, GripperCommand, '/a200_0000/robotiq_85_controller/gripper_cmd')
