...
public:
  ...
  Feedback RefreshFeedback(const MessageId& messageid, uint32_t deviceId = 0,
                           const RouterClientSendOptions& options = { false, 0, 3000 });
  void RefreshFeedback_callback(const MessageId& messageid, std::function<void(const Error&, const Feedback&)> callback,
                                uint32_t deviceId = 0);
  std::future<Feedback> RefreshFeedback_async(const MessageId& messageid, uint32_t deviceId = 0,
                                              const RouterClientSendOptions& options = { false, 0, 3000 });

  CustomData RefreshCustomData(const MessageId& messageid, uint32_t deviceId = 0,
                               const RouterClientSendOptions& options = { false, 0, 3000 });
  void RefreshCustomData_callback(const MessageId& messageid,
                                  std::function<void(const Error&, const CustomData&)> callback, uint32_t deviceId = 0);
  std::future<CustomData> RefreshCustomData_async(const MessageId& messageid, uint32_t deviceId = 0,
                                                  const RouterClientSendOptions& options = { false, 0, 3000 });
