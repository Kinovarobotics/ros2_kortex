...
class ExampleVisionConfiguration:
    def __init__(self):
        try:
            ...
            # Init the services
            get_intrinsic_parameters_full_name = (
                "/" + self.robot_name + "/vision_config/get_intrinsic_parameters"
            )
            rospy.wait_for_service(get_intrinsic_parameters_full_name, 0.5)
            self.get_intrinsic_parameters = rospy.ServiceProxy(
                get_intrinsic_parameters_full_name, GetIntrinsicParameters
            )

            get_extrinsic_parameters_full_name = (
                "/" + self.robot_name + "/vision_config/get_extrinsic_parameters"
            )
            rospy.wait_for_service(get_extrinsic_parameters_full_name, 0.5)
            self.get_extrinsic_parameters = rospy.ServiceProxy(
                get_extrinsic_parameters_full_name, GetExtrinsicParameters
            )

            get_sensor_settings_full_name = (
                "/" + self.robot_name + "/vision_config/get_sensor_settings"
            )
            rospy.wait_for_service(get_sensor_settings_full_name, 0.5)
            self.get_sensor_settings = rospy.ServiceProxy(
                get_sensor_settings_full_name, GetSensorSettings
            )

            set_sensor_settings_full_name = (
                "/" + self.robot_name + "/vision_config/set_sensor_settings"
            )
            rospy.wait_for_service(set_sensor_settings_full_name, 0.5)
            self.set_sensor_settings = rospy.ServiceProxy(
                set_sensor_settings_full_name, SetSensorSettings
            )

            get_option_value_full_name = (
                "/" + self.robot_name + "/vision_config/get_option_value"
            )
            rospy.wait_for_service(get_option_value_full_name, 0.5)
            self.get_option_value = rospy.ServiceProxy(
                get_option_value_full_name, GetOptionValue
            )
        except:
    ...
    def example_get_intrinsic_parameters(self):
        ...
        req.input.sensor = (
            Sensor.SENSOR_COLOR
        )  # Change to SENSOR_DEPTH for the depth sensor
        try:
            ...
            out += (
                "Focal length in x is : "
                + str(intrinsic_parameters.focal_length_x)
                + "\n"
            )
            out += (
                "Focal length in y is : "
                + str(intrinsic_parameters.focal_length_y)
                + "\n"
            )
            out += (
                "Principal point in x is : "
                + str(intrinsic_parameters.principal_point_x)
                + "\n"
            )
            out += (
                "Principal point in y is : "
                + str(intrinsic_parameters.principal_point_y)
                + "\n"
            )
            ...
            out += (
                "Sensor type is : "
                + self.sensor_type_enum_to_string(intrinsic_parameters.sensor)
                + "\n"
            )
            ...
            out += (
                "Resolution is : "
                + self.resolution_enum_to_string(intrinsic_parameters.resolution)
                + "\n"
            )
            out += "----------------------------------"
    ...
    def example_get_extrinsic_parameters(self):
        ...
        try:
            ...
            out += (
                "|  "
                + str(extrinsic_parameters.rotation.row1.column1)
                + "  ;  "
                + str(extrinsic_parameters.rotation.row1.column2)
                + "  ;  "
                + str(extrinsic_parameters.rotation.row1.column3)
                + "  |"
                + "\n"
            )
            out += (
                "|  "
                + str(extrinsic_parameters.rotation.row2.column1)
                + "  ;  "
                + str(extrinsic_parameters.rotation.row2.column2)
                + "  ;  "
                + str(extrinsic_parameters.rotation.row2.column3)
                + "  |"
                + "\n"
            )
            out += (
                "|  "
                + str(extrinsic_parameters.rotation.row3.column1)
                + "  ;  "
                + str(extrinsic_parameters.rotation.row3.column2)
                + "  ;  "
                + str(extrinsic_parameters.rotation.row3.column3)
                + "  |"
                + "\n"
            )
    ...
    def example_get_sensor_settings(self):
        ...
        try:
            ...
            out += (
                "Bit rate : "
                + self.bitrate_enum_to_string(sensor_settings.bit_rate)
                + "\n"
            )
            out += (
                "Frame rate : "
                + self.framerate_enum_to_string(sensor_settings.frame_rate)
                + "\n"
            )
            out += (
                "Resolution : "
                + self.resolution_enum_to_string(sensor_settings.resolution)
                + "\n"
            )
            out += "---------------------------------"
    ...
    def example_get_sensor_option_value(self):
        ...
        try:
            ...
            out += (
                "For sensor : "
                + self.sensor_type_enum_to_string(option_value.sensor)
                + "\n"
            )
    ...
    def main(self):
        ...
        try:
            rospy.delete_param(
                "/kortex_examples_test_results/vision_configuration_python"
            )
        except:
            pass
        ...
        # For testing purposes
        rospy.set_param(
            "/kortex_examples_test_results/vision_configuration_python", success
        )
