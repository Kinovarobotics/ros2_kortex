...
class ExampleActuatorConfiguration:
    def __init__(self):
        try:
            ...
            # Init the services
            read_all_devices_full_name = (
                "/" + self.robot_name + "/device_manager/read_all_devices"
            )
            rospy.wait_for_service(read_all_devices_full_name, 0.5)
            self.read_all_devices = rospy.ServiceProxy(
                read_all_devices_full_name, ReadAllDevices
            )

            set_device_id_full_name = (
                "/" + self.robot_name + "/actuator_config/set_device_id"
            )
            rospy.wait_for_service(set_device_id_full_name, 0.5)
            self.set_device_id = rospy.ServiceProxy(
                set_device_id_full_name, SetDeviceID
            )

            clear_faults_full_name = (
                "/" + self.robot_name + "/actuator_config/clear_faults"
            )
            rospy.wait_for_service(clear_faults_full_name, 0.5)
            self.clear_faults = rospy.ServiceProxy(
                clear_faults_full_name, ActuatorConfig_ClearFaults
            )
        ...
        try:
            ...
            for device in all_devices.output.device_handle:
                if (
                    ...
                    rospy.loginfo(
                        "Found an actuator with device id "
                        + str(device.device_identifier)
                    )
            ...
            if self.device_id in actuator_device_ids:
                rospy.loginfo(
                    "Device id "
                    + str(self.device_id)
                    + " is a valid actuator device id."
                )
            else:
                raise ValueError(
                    "Device id "
                    + str(self.device_id)
                    + " does not correspond to an actuator's device id."
                )
                return False
    ...
    def example_clear_actuator_faults(self):
        try:
            self.clear_faults()
            rospy.loginfo(
                "Faults were cleared successfully on actuator " + str(self.device_id)
            )
            return True
        except rospy.ServiceException:
    ...
    def example_get_control_loop_parameters(self, output):
        try:
            ...
            oss = (
                oss
                + "Loop selection : "
                + self.control_loop_to_string(control_loop_parameters.loop_selection)
                + "\n"
            )
            oss = (
                oss
                + "Error saturation : "
                + str(control_loop_parameters.error_saturation)
                + "\n"
            )
            oss = (
                oss
                + "Output saturation : "
                + str(control_loop_parameters.output_saturation)
                + "\n"
            )
            oss = oss + "kAz : ["
            ...
            oss = (
                oss
                + "Error dead band : "
                + str(control_loop_parameters.error_dead_band)
                + "\n"
            )
            rospy.loginfo(oss)
    ...
    def main(self):
        ...
        # For testing purposes
        rospy.set_param(
            "/kortex_examples_test_results/actuator_configuration_python", success
        )
