...
FORBIDDEN_RPC_METHODS = [
    ...
    "WriteTorqueCalibration",
    "GetVectorDriveParameters",
    "SetVectorDriveParameters",
    "GetEncoderDerivativeParameters",
    "SetEncoderDerivativeParameters",
    "StartFrequencyResponse",
...
class DetailedMessage:
    ...
    def __init__(self, message, package, is_message_duplicated, duplicatedFields=[]):
        ...
        self.name_lowercase_with_underscores = "_".join(
            re.findall("[A-Z][^A-Z]*", self.name)
        ).lower()
        ...
        self.containing_folder = (
            "_".join(re.findall("[A-Z][^A-Z]*", package.split(".")[-1])).lower() + "/"
        )
...
class DetailedPackage:
    ...
    def __init__(self, package):
        ...
        self.short_name_lowercase_with_underscores = "_".join(
            re.findall("[A-Z][^A-Z]*", self.short_name)
        ).lower()
...
class DetailedRPC:
    ...
    def __init__(
        ...
        notification_message_cpp_namespace=None,
    ):
        """
        ...
        if is_input_type_duplicated:
            self.ros_service_input_name = (
                rpc.input_type.split(".")[-2] + "_" + rpc.input_type.split(".")[-1]
            )
        else:
        ...
        if is_output_type_duplicated:
            self.ros_service_output_name = (
                rpc.output_type.split(".")[-2] + "_" + rpc.output_type.split(".")[-1]
            )
        else:
        ...
        self.prepend_on_notification = (
            "OnNotification" if self.is_notification_rpc else ""
        )
...
def generate_code(request, response):
    ...
    for filename, proto_file in file_map.items():
        ...
        proto_file_reader = open(
            str(PurePath(PROTO_FILES_RELATIVE_PATH, filename)),
            encoding="utf8",
            mode="r",
        )
        ...
        while text_line != "":
            ...
            if "rpc " in text_line:
                ...
                if re.search(r"@DEPRECATED", text_line):
                    ...
                    deprecated_rpcs_dict_of_lists[filename_without_extension].append(
                        rpc_name
                    )
    ...
    for filename, proto_file in file_map.items():
        ...
        for item, package in traverse(proto_file):
            ...
            if isinstance(item, EnumDescriptorProto):
                is_enum_duplicated = (
                    item.name.casefold() in duplicated_messages_unordered_set
                )
            ...
            if isinstance(item, DescriptorProto):
                ...
                duplicated_fields = filter(
                    lambda x: x.type_name.split(".")[-1].casefold()
                    in duplicated_messages_unordered_set,
                    item.field,
                )
                temp_message = DetailedMessage(
                    item, package, is_message_duplicated, duplicated_fields
                )
                ...
                if message_contains_one_of:
                    ...
                    for one_of in item.ListFields()[-1][
                        1
                    ]:  # This is the very obscure way to get the one_of's name
                    ...
                    for member in item.field:
                        ...
                        if member.HasField("oneof_index"):
                            is_field_duplicated = (
                                member.type_name.split(".")[-1].casefold()
                                in duplicated_messages_unordered_set
                            )
                            temp_message.one_of_list[member.oneof_index].addField(
            ...
            if isinstance(item, ServiceDescriptorProto):
                ...
                deprecated_rpcs_in_this_service = deprecated_rpcs_dict_of_lists[
                    item.name
                ]
                ...
                for rpc in item.method:
                    ...
                    is_input_type_duplicated = (
                        rpc.input_type.split(".")[-1].casefold()
                        in duplicated_messages_unordered_set
                    )
                    is_output_type_duplicated = (
                        rpc.output_type.split(".")[-1].casefold()
                        in duplicated_messages_unordered_set
                    )
                    is_rpc_deprecated = (
                        True if rpc.name in deprecated_rpcs_in_this_service else False
                    )
                    temp_rpc = DetailedRPC(
                        rpc,
                        package,
                        is_rpc_duplicated,
                        is_rpc_deprecated,
                    ...
                    if temp_rpc.is_notification_rpc:
                        notification_message = notification_messages_map[
                            "{}.{}".format(package, rpc.name)
                        ]
                        notification_message_cpp_namespace = "::".join(
                            notification_message.split(".")[:-1]
                        )
                        temp_rpc.set_notification_cpp_namespace(
                            notification_message_cpp_namespace
                        )
    ...
    packages_with_messages = filter(
        lambda x: len(x.messages) > 0, packages_dict.values()
    )
    include_file_names = []
    for p in packages_with_messages:
        for s in ["proto", "ros"]:
            include_file_names.append(
                "kortex_driver/generated/robot/{}_{}_converter.h".format(
    ...
    for package in packages_dict.values():
        ...
        for enum in package.enums:
            ...
            ros_enum_path = os.path.join(
                "..",
                "msg/generated/{}{}{}.msg".format(
                    enum.containing_folder, enum.prepend_message_name, enum.name
                ),
            )
            with open(ros_enum_path, "wt") as serviceFile:
                serviceFile.write(
                    render(
                        "../templates/ros_enum.msg.jinja2", this_enum_context.__dict__
                    )
                )
        ...
        for message in package.messages:
            ...
            for detailed_one_of in message.one_of_list:  # not empty
                ...
                with open(ros_oneofPath, "wt") as serviceFile:
                    serviceFile.write(
                        render(
                            "../templates/ros_oneof.msg.jinja2",
            ...
            with open(ros_messagePath, "wt") as serviceFile:
                serviceFile.write(
                    render(
                        "../templates/ros_message.msg.jinja2",
                        this_message_context.__dict__,
                    )
                )
        ...
        for rpc in package.methods:
            ...
            ros_servicePath = os.path.join(
                ...
                "srv/generated/{}/{}{}{}.srv".format(
                    ...
                    rpc.prepend_rpc_package_name,
                    rpc.prepend_on_notification,
                    rpc.name,
                ),
            )
            with open(ros_servicePath, "wt") as serviceFile:
                serviceFile.write(
                    render(
                        "../templates/ros_service.srv.jinja2", this_rpc_context.__dict__
                    )
                )
        ...
        if package.messages:  # package contains at least one message
            ...
            current_header_filename = (
                "kortex_driver/generated/robot/{}_proto_converter.h".format(
                    package.short_name.lower()
                )
            )
            this_package_context.include_file_names = filter(
                lambda x: "proto_converter" in x and x != current_header_filename,
                include_file_names,
            )
            with open(
                os.path.join("..", "include/" + current_header_filename), "wt"
            ) as converterFile:
                converterFile.write(
                    render(
                        "../templates/proto_converter.h.jinja2",
            ...
            with open(
                os.path.join(
                    "..",
                    "src/generated/robot/{}_proto_converter.cpp".format(
                        package.short_name.lower()
                    ),
                ),
                "wt",
                ...
                converterFile.write(
                    render(
                        "../templates/proto_converter.cpp.jinja2",
                        this_package_context.__dict__,
                    )
                )
            # ROS converter header file
            current_header_filename = (
                "kortex_driver/generated/robot/{}_ros_converter.h".format(
                    package.short_name.lower()
                )
            )
            this_package_context.include_file_names = filter(
                lambda x: "ros_converter" in x and x != current_header_filename,
                include_file_names,
            )
            with open(
                os.path.join("..", "include/" + current_header_filename), "wt"
                ...
                converterFile.write(
                    render(
                        "../templates/ros_converter.h.jinja2",
                        this_package_context.__dict__,
                    )
                )
            ...
            with open(
                os.path.join(
                    "..",
                    "src/generated/robot/{}_ros_converter.cpp".format(
                        package.short_name.lower()
                    ),
                ),
                "wt",
                ...
                converterFile.write(
                    render(
                        "../templates/ros_converter.cpp.jinja2",
                        this_package_context.__dict__,
                    )
                )
        ...
        if package.methods:  # package contains at least one RPC
            ...
            current_header_filename = (
                "kortex_driver/generated/interfaces/{}_services_interface.h".format(
                    package.short_name.lower()
                )
            )
            current_interface_header_filename = current_header_filename
            this_package_context.current_interface_header_filename = (
            ...
            with open(
                ...
                services_file.write(
                    render(
                        "../templates/services_interface.h.jinja2",
                        this_package_context.__dict__,
                    )
                )
            ...
            current_header_filename = (
                "kortex_driver/generated/robot/{}_services.h".format(
                    package.short_name.lower()
                )
            )
            this_package_context.current_header_filename = current_header_filename
            ...
            with open(
                ...
                services_file.write(
                    render(
                        "../templates/services_robot.h.jinja2",
                        this_package_context.__dict__,
                    )
                )
            with open(
                os.path.join(
                    "..",
                    "src/generated/robot/{}_services.cpp".format(
                        package.short_name.lower()
                    ),
                ),
                "wt",
                ...
                services_file.write(
                    render(
                        "../templates/services_robot.cpp.jinja2",
                        this_package_context.__dict__,
                    )
                )
            ...
            current_header_filename = (
                "kortex_driver/generated/simulation/{}_services.h".format(
                    package.short_name.lower()
                )
            )
            this_package_context.current_header_filename = current_header_filename
            ...
            with open(
                ...
                services_file.write(
                    render(
                        "../templates/services_simulation.h.jinja2",
                        this_package_context.__dict__,
                    )
                )
            with open(
                os.path.join(
                    "..",
                    "src/generated/simulation/{}_services.cpp".format(
                        package.short_name.lower()
                    ),
                ),
                "wt",
                ...
                services_file.write(
                    render(
                        "../templates/services_simulation.cpp.jinja2",
                        this_package_context.__dict__,
                    )
                )
