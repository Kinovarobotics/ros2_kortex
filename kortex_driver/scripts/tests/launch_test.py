...
parser = argparse.ArgumentParser(
    description="Launch ROS examples or ROS driver automated tests for a given arm configuration."
)
parser.add_argument(
    "--no-vision",
    action="store_true",
    help="Add this option if the arm doesn't have a vision module",
)
parser.add_argument(
    "ip", type=str, help="Arm IP address (enter 'none' if testing Gazebo)"
)
...
parser.add_argument(
    "gripper",
    choices=["robotiq_2f_85", "robotiq_2f_140", "gen3_lite_2f", "none"],
    help="Gripper type",
)
parser.add_argument(
    "test_case", choices=["driver", "examples", "gazebo"], help="Test case to run"
)
...
try:
    ...
    # If we want to test the examples
        ...
        time.sleep(10)
        examples_tests_process = subprocess.Popen(
            [
                "rostest",
                "--reuse-master",
                "kortex_examples",
                "run_all_examples.launch",
                "robot_name:=my_{}".format(args.arm),
            ]
        )
