...
KortexArmSimulation::KortexArmSimulation(ros::NodeHandle& node_handle)
  ...
  , m_active_controller_type{ ControllerType::kTrajectory }
{
  ...
  for (int i = 0; i < GetDOF(); i++)
  {
    m_velocity_trap_profiles.push_back(
        KDL::VelocityProfile_Trap(m_arm_velocity_max_limits[i], m_arm_acceleration_max_limits[i]));
  }
  ...
  m_sub_joint_state = m_node_handle.subscribe("/" + m_robot_name + "/" + "joint_states", 1,
                                              &KortexArmSimulation::cb_joint_states, this);
  ...
  m_trajectory_controllers_list.push_back(m_prefix + m_arm_name +
                                          "_joint_trajectory_controller");  // only one trajectory controller
  m_position_controllers_list.resize(GetDOF());                             // one position controller per
  std::generate(m_position_controllers_list.begin(), m_position_controllers_list.end(), [this]() -> std::string {
    static int i = 1;
  ...
  m_follow_joint_trajectory_action_client.reset(
...
kortex_driver::BaseCyclic_Feedback KortexArmSimulation::GetFeedback()
{
  ...
  for (int i = 0; i < GetDOF(); i++)
  {
    m_feedback.actuators[i].position =
        m_math_util.wrapDegreesFromZeroTo360(m_math_util.toDeg(current.position[m_first_arm_joint_index + i]));
  ...
  if (IsGripperPresent())
  {
    // Gripper index is right after last actuator and is expressed in % in the base feedback (not in absolute position
    // like in joint_states)
    m_feedback.interconnect.oneof_tool_feedback.gripper_feedback[0].motor[0].position =
        100.0 * m_math_util.relative_position_from_absolute(current.position[m_gripper_joint_index],
                                                            m_gripper_joint_limits_min[0],
                                                            m_gripper_joint_limits_max[0]);
  }
...
kortex_driver::ReadAllActions::Response
KortexArmSimulation::ReadAllActions(const kortex_driver::ReadAllActions::Request& req)
{
  auto input = req.input;
...
kortex_driver::ExecuteActionFromReference::Response
KortexArmSimulation::ExecuteActionFromReference(const kortex_driver::ExecuteActionFromReference::Request& req)
{
  auto handle = req.input;
  auto it = m_map_actions.find(handle.identifier);
  if (it != m_map_actions.end())
...
kortex_driver::ExecuteAction::Response
KortexArmSimulation::ExecuteAction(const kortex_driver::ExecuteAction::Request& req)
{
  auto action = req.input;
  // Add Action to map if type is supported
...
kortex_driver::PlayCartesianTrajectory::Response
KortexArmSimulation::PlayCartesianTrajectory(const kortex_driver::PlayCartesianTrajectory::Request& req)
{
...
kortex_driver::SendTwistCommand::Response
KortexArmSimulation::SendTwistCommand(const kortex_driver::SendTwistCommand::Request& req)
{
...
kortex_driver::PlayJointTrajectory::Response
KortexArmSimulation::PlayJointTrajectory(const kortex_driver::PlayJointTrajectory::Request& req)
{
...
kortex_driver::SendJointSpeedsCommand::Response
KortexArmSimulation::SendJointSpeedsCommand(const kortex_driver::SendJointSpeedsCommand::Request& req)
{
  ...
  int n = 0;
  std::generate(m_velocity_commands.begin(), m_velocity_commands.end(), [this, &action, &n]() -> double {
...
kortex_driver::SendGripperCommand::Response
KortexArmSimulation::SendGripperCommand(const kortex_driver::SendGripperCommand::Request& req)
{
...
kortex_driver::ApplyEmergencyStop::Response
KortexArmSimulation::ApplyEmergencyStop(const kortex_driver::ApplyEmergencyStop::Request& req)
{
  // If an action is ongoing, cancel it first
  if (m_is_action_being_executed.load())
  {
...
kortex_driver::KortexError KortexArmSimulation::FillKortexError(uint32_t code, uint32_t subCode,
...
{
  auto action_result =
      FillKortexError(kortex_driver::ErrorCodes::ERROR_NONE, kortex_driver::SubErrorCodes::SUB_ERROR_NONE);
  ...
  switch (action.handle.action_type)
  {
    ...
    default:
      action_result =
          FillKortexError(kortex_driver::ErrorCodes::ERROR_DEVICE, kortex_driver::SubErrorCodes::UNSUPPORTED_ACTION);
      break;
  }
    ...
    else
    {
      if (action_result.code != kortex_driver::ErrorCodes::ERROR_NONE)
      {
        ...
        ROS_WARN("Action was failed : \nError code is %d\nSub-error code is %d\nError description is : %s",
                 action_result.code, action_result.subCode, action_result.description.c_str());
      }
      else
      {
        // Notify ACTION_END
...
kortex_driver::KortexError KortexArmSimulation::ExecuteReachJointAngles(const kortex_driver::Action& action)
{
  ...
  {
    return FillKortexError(kortex_driver::ErrorCodes::ERROR_DEVICE, kortex_driver::SubErrorCodes::INVALID_PARAM,
  ...
  {
    return FillKortexError(kortex_driver::ErrorCodes::ERROR_DEVICE, kortex_driver::SubErrorCodes::INVALID_PARAM,
                           "Error playing joint angles action : action contains " +
                               std::to_string(constrained_joint_angles.joint_angles.joint_angles.size()) +
                               " joint angles but arm has " + std::to_string(GetDOF()));
  }

  // Switch to trajectory controller
  if (!SwitchControllerType(ControllerType::kTrajectory))
  {
    return FillKortexError(
        kortex_driver::ErrorCodes::ERROR_DEVICE, kortex_driver::SubErrorCodes::METHOD_FAILED,
  ...
  for (int i = 0; i < constrained_joint_angles.joint_angles.joint_angles.size(); i++)
  {
    ...
    const double rad_wrapped_goal = m_math_util.wrapRadiansFromMinusPiToPi(
        m_math_util.toRad(constrained_joint_angles.joint_angles.joint_angles[i].value));
  ...
  switch (constrained_joint_angles.constraint.type)
  {
    ...
    case kortex_driver::JointTrajectoryConstraintType::JOINT_CONSTRAINT_DURATION:
    {
      ...
      for (int i = 0; i < GetDOF(); i++)
      {
        m_velocity_trap_profiles[i].SetProfileDuration(current.position[m_first_arm_joint_index + i],
                                                       endpoint.positions[i],
                                                       constrained_joint_angles.constraint.value);
      }
    ...
    case kortex_driver::JointTrajectoryConstraintType::JOINT_CONSTRAINT_SPEED:
    {
      ...
      for (int i = 0; i < GetDOF(); i++)
      {
        ...
        m_velocity_trap_profiles[i].SetProfileVelocity(current.position[m_first_arm_joint_index + i],
                                                       endpoint.positions[i], velocity_ratio);
        max_duration = std::max(max_duration, m_velocity_trap_profiles[i].Duration());
        ROS_DEBUG("Joint %d moving from %2.2f to %2.2f gives duration %2.2f", i,
                  current.position[m_first_arm_joint_index + i], endpoint.positions[i],
                  m_velocity_trap_profiles[i].Duration());
      }
      ...
      {
        m_velocity_trap_profiles[i].SetProfileDuration(current.position[m_first_arm_joint_index + i],
                                                       endpoint.positions[i], max_duration);
      }
    ...
    default:
    {
      ...
      {
        m_velocity_trap_profiles[i].SetProfileDuration(current.position[m_first_arm_joint_index + i],
                                                       endpoint.positions[i], optimal_duration);
      }
  ...
  // For each timestep
  for (double t = JOINT_TRAJECTORY_TIMESTEP_SECONDS; t < m_velocity_trap_profiles[0].Duration();
       t += JOINT_TRAJECTORY_TIMESTEP_SECONDS)
  {
  ...
  else
  {
    ...
    {
      result = FillKortexError(kortex_driver::ErrorCodes::ERROR_DEVICE, kortex_driver::SubErrorCodes::INVALID_PARAM,
                               status->error_string);
    }
  }
  return result;
...
kortex_driver::KortexError KortexArmSimulation::ExecuteReachPose(const kortex_driver::Action& action)
{
  ...
  if (!SwitchControllerType(ControllerType::kTrajectory))
  {
    return FillKortexError(kortex_driver::ErrorCodes::ERROR_DEVICE, kortex_driver::SubErrorCodes::METHOD_FAILED,
  ...
  // Get End frame
  auto end_pos =
      KDL::Vector(constrained_pose.target_pose.x, constrained_pose.target_pose.y, constrained_pose.target_pose.z);
  auto end_rot = KDL::Rotation::EulerZYX(m_math_util.toRad(constrained_pose.target_pose.theta_z),
                                         m_math_util.toRad(constrained_pose.target_pose.theta_y),
                                         m_math_util.toRad(constrained_pose.target_pose.theta_x));
  ...
  {
    // If a max velocity is supplied for each joint, we need to find the limiting duration with this velocity constraint
    translation_speed_limit =
        std::min(constrained_pose.constraint.oneof_type.speed[0].translation, translation_speed_limit);
  ...
  KDL::Vector
      axis;  // we need to create this variable to access the RotAngle for start and end frames'rotation components
  ...
  // https://github.com/zakharov/BRICS_RN/blob/master/navigation_trajectory_common/include/navigation_trajectory_common/Conversions.h#L358-L382
  ...
  KDL::VelocityProfile_Trap velocity_profile(std::min(translation_speed_limit, rotation_speed_limit),
                                             m_max_cartesian_acceleration_linear);
  ...
  for (float t = JOINT_TRAJECTORY_TIMESTEP_SECONDS; t < duration; t += JOINT_TRAJECTORY_TIMESTEP_SECONDS)
  {
    ...
    // Position is enough to have a smooth trajectory it seems but if eventually vel and acc are useful somehow this is
    // how to get them: auto vel = segment.Vel(t); auto acc = segment.Acc(t);
...
kortex_driver::KortexError KortexArmSimulation::ExecuteSendJointSpeeds(const kortex_driver::Action& action)
{
  ...
  {
    return FillKortexError(kortex_driver::ErrorCodes::ERROR_DEVICE, kortex_driver::SubErrorCodes::INVALID_PARAM,
                           "Error playing joint speeds action : action contains " +
                               std::to_string(joint_speeds.joint_speeds.size()) + " joint speeds but arm has " +
  ...
  while (true)
  {
    ...
    for (int i = 0; i < GetDOF(); i++)
    {
      ...
      velocity_command =
          std::copysign(std::min(fabs(velocity_command), double(fabs(m_arm_velocity_max_limits[i]))), velocity_command);
...
kortex_driver::KortexError KortexArmSimulation::ExecuteSendTwist(const kortex_driver::Action& action)
{
  ...
  while (ros::ok())
  {
    ...
    {
      twist_command.linear_x =
          previous_twist_command.linear_x + std::copysign(max_linear_twist_delta, delta_twist.linear_x);
    }
    // same for linear_y
    ...
    {
      twist_command.linear_y =
          previous_twist_command.linear_y + std::copysign(max_linear_twist_delta, delta_twist.linear_y);
    }
    // same for linear_z
    ...
    {
      twist_command.linear_z =
          previous_twist_command.linear_z + std::copysign(max_linear_twist_delta, delta_twist.linear_z);
    }
    // same for angular_x
    ...
    {
      twist_command.angular_x =
          previous_twist_command.angular_x + std::copysign(max_angular_twist_delta, delta_twist.angular_x);
    }
    // same for angular_y
    ...
    {
      twist_command.angular_y =
          previous_twist_command.angular_y + std::copysign(max_angular_twist_delta, delta_twist.angular_y);
    }
    // same for angular_z
    ...
    {
      twist_command.angular_z =
          previous_twist_command.angular_z + std::copysign(max_angular_twist_delta, delta_twist.angular_z);
    }

    // Cap to the velocity limit
    twist_command.linear_x = std::copysign(std::min(fabs(twist_command.linear_x), fabs(m_max_cartesian_twist_linear)),
                                           twist_command.linear_x);
    twist_command.linear_y = std::copysign(std::min(fabs(twist_command.linear_y), fabs(m_max_cartesian_twist_linear)),
                                           twist_command.linear_y);
    twist_command.linear_z = std::copysign(std::min(fabs(twist_command.linear_z), fabs(m_max_cartesian_twist_linear)),
                                           twist_command.linear_z);
    twist_command.angular_x = std::copysign(
        std::min(fabs(twist_command.angular_x), fabs(m_max_cartesian_twist_angular)), twist_command.angular_x);
    twist_command.angular_y = std::copysign(
        std::min(fabs(twist_command.angular_y), fabs(m_max_cartesian_twist_angular)), twist_command.angular_y);
    twist_command.angular_z = std::copysign(
        std::min(fabs(twist_command.angular_z), fabs(m_max_cartesian_twist_angular)), twist_command.angular_z);
    ...
    KDL::Twist twist_kdl =
        KDL::Twist(KDL::Vector(twist_command.linear_x, twist_command.linear_y, twist_command.linear_z),
...
kortex_driver::KortexError KortexArmSimulation::ExecuteSendGripperCommand(const kortex_driver::Action& action)
{
  ...
  {
    return FillKortexError(kortex_driver::ErrorCodes::ERROR_DEVICE, kortex_driver::SubErrorCodes::UNSUPPORTED_ACTION,
                           "Error playing gripper command action : gripper mode " +
                               std::to_string(gripper_command.mode) + " is not supported; only position is.");
  }

  // The incoming command is relative [0,1] and we need to put it in absolute unit [m_gripper_joint_limits_min[0],
  // m_gripper_joint_limits_max[0]]:
  double absolute_gripper_command = m_math_util.absolute_position_from_relative(
      gripper_command.gripper.finger[0].value, m_gripper_joint_limits_min[0], m_gripper_joint_limits_max[0]);
  ...
  else
  {
    ...
    {
      result = FillKortexError(kortex_driver::ErrorCodes::ERROR_DEVICE, kortex_driver::SubErrorCodes::METHOD_FAILED,
...
kortex_driver::KortexError KortexArmSimulation::ExecuteTimeDelay(const kortex_driver::Action& action)
{
  ...
  if (!action.oneof_action_parameters.delay.empty())
  {
    ...
    while (!m_action_preempted.load() &&
           (std::chrono::system_clock::now() - start) < std::chrono::seconds(delay_seconds))
    {
      // sleep a bit
