...
JointTrajectoryActionServer::JointTrajectoryActionServer(const std::string& server_name, ros::NodeHandle& nh,
                                                         Kinova::Api::Base::BaseClient* base,
                                                         Kinova::Api::BaseCyclic::BaseCyclicClient* base_cyclic,
                                                         Kinova::Api::ControlConfig::ControlConfigClient* control_config,
                                                         bool use_hard_limits)
  : m_server_name(server_name)
  , m_node_handle(nh)
  , m_server(nh, server_name, boost::bind(&JointTrajectoryActionServer::goal_received_callback, this, _1),
             boost::bind(&JointTrajectoryActionServer::preempt_received_callback, this, _1), false)
  , m_base(base)
  ...
  , m_use_hard_limits(use_hard_limits)
{
  ...
  m_sub_action_notif_handle = m_base->OnNotificationActionTopic(
      std::bind(&JointTrajectoryActionServer::action_notif_callback, this, std::placeholders::_1),
...
void JointTrajectoryActionServer::goal_received_callback(
    actionlib::ActionServer<control_msgs::FollowJointTrajectoryAction>::GoalHandle new_goal_handle)
{
  ...
  for (unsigned int i = 0; i < new_goal_handle.getGoal()->trajectory.points.size(); i++)
  {
    ...
    // FIXME We have to include the starting point for the trajectory as the first waypoint AND since optimal durations
    // are not
    //       supported yet, we also have to give it a non-zero duration. Putting something too small gives errors so
    //       I've put an arbitrary value of 500ms here for now.
    if (i == 0)
    {
  ...
  try
  {
    ...
    if (report.trajectory_error_report().trajectory_error_elements_size() > 0)
    {
      ...
      {
        ROS_ERROR("Error %i : %s", i + 1,
                  report.trajectory_error_report().trajectory_error_elements(i).message().c_str());
      }
...
void JointTrajectoryActionServer::preempt_received_callback(
    actionlib::ActionServer<control_msgs::FollowJointTrajectoryAction>::GoalHandle goal_handle)
{
  ...
  if (type == Kinova::Api::Base::ActionType::EXECUTE_WAYPOINT_LIST)
  {
    switch (event)
    {
      ...
      case Kinova::Api::Base::ActionEvent::ACTION_PREPROCESS_START:
        ...
        {
          ROS_DEBUG("Notification mismatch : received ACTION_PREPROCESS_START but we are in %s",
                    actionServerStateNames[int(m_server_state)]);
        }
        break;
      ...
      case Kinova::Api::Base::ActionEvent::ACTION_PREPROCESS_END:
        ...
        {
          ROS_DEBUG("Notification mismatch : received ACTION_PREPROCESS_END but we are in %s",
                    actionServerStateNames[int(m_server_state)]);
        }
        break;
      ...
      case Kinova::Api::Base::ActionEvent::ACTION_PREPROCESS_ABORT:
        ...
        {
          ROS_ERROR("Preprocessing has finished in the arm and goal has been rejected. Fetching the error report from "
                    "the arm...");
          ...
          for (auto error_element : report.trajectory_error_elements())
          {
            ...
            oss << "Erroneous value is " << error_element.error_value() << " but minimum permitted is "
                << error_element.min_value() << " and maximum permitted is " << error_element.max_value() << std::endl;
        ...
        {
          ROS_DEBUG("Notification mismatch : received ACTION_PREPROCESS_ABORT but we are in %s",
                    actionServerStateNames[int(m_server_state)]);
        }
        break;
      ...
      case Kinova::Api::Base::ActionEvent::ACTION_START:
        ...
        if ((m_server_state == ActionServerState::TRAJECTORY_EXECUTION_PENDING ||
             m_server_state ==
                 ActionServerState::PRE_PROCESSING_IN_PROGRESS) &&  // FIXME KOR-3563 this happens if we received a
                                                                    // ACTION_START before a ACTION_PREPROCESS_END
        ...
        {
          ROS_DEBUG("Notification mismatch : received ACTION_START but we are in %s",
                    actionServerStateNames[int(m_server_state)]);
        }
        break;
      ...
      case Kinova::Api::Base::ActionEvent::ACTION_ABORT:
             ...
             m_server_state == ActionServerState::TRAJECTORY_EXECUTION_PENDING))
        {
          ...
          {
            oss << "The starting point for the trajectory did not match the actual commanded joint angles."
                << std::endl;
          }
          else if (notif.abort_details() == Kinova::Api::SubErrorCodes::CONTROL_MANUAL_STOP)
          {
            oss << "The speed while executing the trajectory was too damn high and caused the robot to stop."
                << std::endl;
          }
          result.error_string = oss.str();
          m_goal.setAborted(result);
        ...
        {
          ROS_DEBUG("Notification mismatch : received ACTION_ABORT but we are in %s",
                    actionServerStateNames[int(m_server_state)]);
        }
        break;
      ...
      case Kinova::Api::Base::ActionEvent::ACTION_END:
      {
        ...
        if ((m_server_state == ActionServerState::TRAJECTORY_EXECUTION_IN_PROGRESS))
        {
          ...
          {
            result.error_code = result.PATH_TOLERANCE_VIOLATED;
            oss << "After validation, trajectory execution failed in the arm with sub error code "
                << Kinova::Api::SubErrorCodes_Name(notif.abort_details());
            result.error_string = oss.str();
        ...
        {
          ROS_DEBUG("Notification mismatch : received ACTION_END but we are in %s",
                    actionServerStateNames[int(m_server_state)]);
        }
        break;
      }
...
bool JointTrajectoryActionServer::is_goal_acceptable(
    actionlib::ActionServer<control_msgs::FollowJointTrajectoryAction>::GoalHandle goal_handle)
{
  ...
  {
    ROS_ERROR("Goal commands %lu actuators, but arm has %lu.", goal->trajectory.joint_names.size(),
              m_joint_names.size());
    return false;
  }
...
bool JointTrajectoryActionServer::is_goal_tolerance_respected(bool enable_prints, bool check_time_tolerance)
{
  ...
  {
    double actual_trajectory_duration =
        std::chrono::duration<double>(m_trajectory_end_time - m_trajectory_start_time).count();
    double desired_trajectory_duration =
        goal->trajectory.points.at(goal->trajectory.points.size() - 1).time_from_start.toSec() +
        STARTING_POINT_ARBITRARY_DURATION;
    double time_tolerance =
        goal->goal_time_tolerance.toSec() == 0.0 ? m_default_goal_time_tolerance : goal->goal_time_tolerance.toSec();
    if (actual_trajectory_duration > desired_trajectory_duration + time_tolerance)
    {
      if (enable_prints)
        ROS_ERROR(
            "Goal duration tolerance was exceeded. Maximum desired duration was %f seconds and actual duration was %f",
            desired_trajectory_duration + time_tolerance, actual_trajectory_duration);
      return false;
    }
    else if (actual_trajectory_duration < desired_trajectory_duration - time_tolerance)
    {
      if (enable_prints)
        ROS_ERROR("Goal duration threshold was not reached. Minimum desired duration was %f seconds and actual "
                  "duration was %f",
                  desired_trajectory_duration - time_tolerance, actual_trajectory_duration);
      return false;
    }
  ...
  {
    ROS_DEBUG("Goal did not specify tolerances, using default tolerance of %f radians for every joint.",
              m_default_goal_tolerance);
  ...
  for (auto act : feedback.actuators())
  {
    ...
    double desired_position = KortexMathUtil::wrapDegreesFromZeroTo360(
        KortexMathUtil::toDeg(goal->trajectory.points.at(goal->trajectory.points.size() - 1).positions[current_index]));
    ...
    double error = KortexMathUtil::wrapDegreesFromZeroTo360(
        std::min(fabs(actual_position - desired_position), fabs(fabs(actual_position - desired_position) - 360.0)));
    if (error > tolerance)
    {
      is_goal_respected = false;
      if (enable_prints)
        ROS_ERROR("The tolerance for joint %u was not met. Desired position is %f and actual position is %f",
                  current_index + 1, desired_position, actual_position);
    }
    current_index++;
  }
