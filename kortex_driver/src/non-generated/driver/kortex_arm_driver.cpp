...
KortexArmDriver::KortexArmDriver(ros::NodeHandle nh)
  : m_node_handle(nh)
...
void KortexArmDriver::parseRosArguments()
{
  ...
  if (m_is_real_robot)
  {
    ...
    if (ros::param::get("/use_sim_time", use_sim_time))
    {
      if (use_sim_time)
      {
        std::string error_string = "ROS parameter /use_sim_time is true : you may experience problems and you should "
                                   "set it to false and restart the driver.";
    ...
    if (!ros::param::get("~ip_address", m_ip_address))
    {
      std::string error_string =
          "IP address of the robot was not specified in the launch file, shutting down the node...";
      ROS_ERROR("%s", error_string.c_str());
      throw new std::runtime_error(error_string);
    }
    ...
    {
      std::string error_string =
          "Username for the robot session was not specified in the launch file, shutting down the node...";
      ROS_ERROR("%s", error_string.c_str());
      throw new std::runtime_error(error_string);
    }
    ...
    {
      std::string error_string =
          "Password for the robot session was not specified in the launch file, shutting down the node...";
      ROS_ERROR("%s", error_string.c_str());
      throw new std::runtime_error(error_string);
    }
    ...
    {
      std::string error_string =
          "Usage of hard limits as soft was not specified in the launch file, shutting down the node...";
      ROS_ERROR("%s", error_string.c_str());
      throw new std::runtime_error(error_string);
    }
    ...
    {
      std::string error_string =
          "API RPC timeout duration was not specified in the launch file, shutting down the node...";
      ROS_ERROR("%s", error_string.c_str());
      throw new std::runtime_error(error_string);
    }
    ...
    {
      std::string error_string =
          "API session inactivity timeout duration was not specified in the launch file, shutting down the node...";
      ROS_ERROR("%s", error_string.c_str());
      throw new std::runtime_error(error_string);
    }
    ...
    {
      std::string error_string =
          "API connection inactivity timeout duration was not specified in the launch file, shutting down the node...";
      ROS_ERROR("%s", error_string.c_str());
      throw new std::runtime_error(error_string);
    }
  }
  ...
  {
    std::string error_string =
        "Publish rate of the cyclic data was not specified in the launch file, shutting down the node...";
    ROS_ERROR("%s", error_string.c_str());
    throw new std::runtime_error(error_string);
  }
  ...
  {
    std::string error_string =
        "Number of degrees of freedom was not specified in the launch file, shutting down the node...";
    ROS_ERROR("%s", error_string.c_str());
    throw new std::runtime_error(error_string);
  }
  ...
  if (isGripperPresent())
  {
    ...
    {
      std::string error_string =
          "Gripper joint min limits were not specified in the launch file, shutting down the node...";
      ROS_ERROR("%s", error_string.c_str());
      throw new std::runtime_error(error_string);
    }
    ...
    {
      std::string error_string =
          "Gripper joint max limits were not specified in the launch file, shutting down the node...";
      ROS_ERROR("%s", error_string.c_str());
      throw new std::runtime_error(error_string);
    }
  }
...
void KortexArmDriver::initApi()
{
  ...
  // Create the routers
  m_tcp_router = new Kinova::Api::RouterClient(m_tcp_transport, [](Kinova::Api::KError err) {
    ROS_ERROR("Kortex API error was encountered with the TCP router: %s", err.toString().c_str());
  });
  m_udp_router = new Kinova::Api::RouterClient(m_udp_transport, [](Kinova::Api::KError err) {
    ROS_ERROR("Kortex API error was encountered with the UDP router: %s", err.toString().c_str());
  });
  ...
  {
    std::string error_string =
        "The node could not connect to the arm. Did you specify the right IP address and is the arm powered on?";
    ROS_ERROR("%s", error_string.c_str());
    throw ex_runtime;
  }
}
...
void KortexArmDriver::verifyProductConfiguration()
{
  ...
  if (m_arm_name == "gen3")
  {
    ...
    {
      std::string error_string = "The arm model specified in the launch file doesn't match the detected arm's model, "
  ...
  {
    std::string error_string = "The degrees of freedom specified in the launch file doesn't match the detected number "
                               "of actuators, shutting down the node...";
    ROS_ERROR("%s", error_string.c_str());
    throw new std::runtime_error(error_string);
  }
  ...
  if (!isGripperPresent())
  {
    if (product_config.end_effector_type() !=
        Kinova::Api::ProductConfiguration::EndEffectorType::END_EFFECTOR_TYPE_NOT_INSTALLED)
    {
      std::string error_string = "The gripper model specified in the launch file doesn't match the detected arm's "
                                 "gripper model, shutting down the node...";
      ROS_ERROR("%s", error_string.c_str());
      throw new std::runtime_error(error_string);
    }
  }
  ...
  {
    if (product_config.end_effector_type() !=
        Kinova::Api::ProductConfiguration::EndEffectorType::END_EFFECTOR_TYPE_ROBOTIQ_2F_85)
    {
      std::string error_string = "The gripper model specified in the launch file doesn't match the detected arm's "
                                 "gripper model, shutting down the node...";
      ROS_ERROR("%s", error_string.c_str());
  ...
  {
    if (product_config.end_effector_type() !=
        Kinova::Api::ProductConfiguration::EndEffectorType::END_EFFECTOR_TYPE_ROBOTIQ_2F_140)
    {
      std::string error_string = "The gripper model specified in the launch file doesn't match the detected arm's "
                                 "gripper model, shutting down the node...";
      ROS_ERROR("%s", error_string.c_str());
  ...
  {
    if (product_config.end_effector_type() !=
        Kinova::Api::ProductConfiguration::EndEffectorType::END_EFFECTOR_TYPE_L31_GRIPPER_2F)
    {
      std::string error_string = "The gripper model specified in the launch file doesn't match the detected arm's "
                                 "gripper model, shutting down the node...";
      ROS_ERROR("%s", error_string.c_str());
  ...
  {
    std::string error_string =
        "The gripper model specified in the launch file is not supported, shutting down the node...";
    ROS_ERROR("%s", error_string.c_str());
    throw new std::runtime_error(error_string);
  }
  ...
  for (auto device : devices.device_handle())
  {
    ...
    {
      ROS_INFO("Actuator device of type %s was found on device identifier %u",
               Kinova::Api::Common::DeviceTypes_Name(device.device_type()).c_str(), device.device_identifier());
    }
...
void KortexArmDriver::initRosServices()
{
  ...
  if (m_is_real_robot)
  {
    m_actuator_config_ros_services =
        new ActuatorConfigRobotServices(m_node_handle, m_actuator_config, 0, m_api_rpc_timeout_ms);
    m_base_ros_services = new BaseRobotServices(m_node_handle, m_base, 0, m_api_rpc_timeout_ms);
    m_control_config_ros_services =
        new ControlConfigRobotServices(m_node_handle, m_control_config, 0, m_api_rpc_timeout_ms);
    m_device_config_ros_services =
        new DeviceConfigRobotServices(m_node_handle, m_device_config, 0, m_api_rpc_timeout_ms);
    m_device_manager_ros_services =
        new DeviceManagerRobotServices(m_node_handle, m_device_manager, 0, m_api_rpc_timeout_ms);
    if (m_is_interconnect_module_present)
    {
      m_interconnect_config_ros_services = new InterconnectConfigRobotServices(
          m_node_handle, m_interconnect_config, m_interconnect_device_id, m_api_rpc_timeout_ms);
    }
    else
    {
    ...
    {
      m_vision_config_ros_services =
          new VisionConfigRobotServices(m_node_handle, m_vision_config, m_vision_device_id, m_api_rpc_timeout_ms);
    }
    else
    {
...
void KortexArmDriver::startActionServers()
{
  ...
  m_action_server_follow_joint_trajectory =
      new JointTrajectoryActionServer(m_arm_name + "_joint_trajectory_controller/follow_joint_trajectory",
                                      m_node_handle, m_base, m_base_cyclic, m_control_config, m_use_hard_limits);
  // Start Gripper Action Server if the arm has a gripper

  m_action_server_follow_cartesian_trajectory = new CartesianTrajectoryActionServer(
      "cartesian_trajectory_controller/follow_cartesian_trajectory", m_node_handle, m_base, m_base_cyclic);

  m_action_server_gripper_command = nullptr;
  if (isGripperPresent())
  {
    m_action_server_gripper_command =
        new RobotiqGripperCommandActionServer(m_gripper_name + "_gripper_controller/gripper_cmd",
                                              m_gripper_joint_names[0], m_gripper_joint_limits_min[0],
                                              m_gripper_joint_limits_max[0], m_node_handle, m_base, m_base_cyclic);
  }
}
...
void KortexArmDriver::publishRobotFeedback()
{
  ...
  while (m_node_is_running)
  {
    ...
    joint_state.name.resize(base_feedback.actuators.size() +
                            base_feedback.interconnect.oneof_tool_feedback.gripper_feedback[0].motor.size());
    joint_state.position.resize(base_feedback.actuators.size() +
                                base_feedback.interconnect.oneof_tool_feedback.gripper_feedback[0].motor.size());
    joint_state.velocity.resize(base_feedback.actuators.size() +
                                base_feedback.interconnect.oneof_tool_feedback.gripper_feedback[0].motor.size());
    joint_state.effort.resize(base_feedback.actuators.size() +
                              base_feedback.interconnect.oneof_tool_feedback.gripper_feedback[0].motor.size());
    ...
    for (int i = 0; i < base_feedback.actuators.size(); i++)
    {
      ...
      joint_state.position[i] =
          m_math_util.wrapRadiansFromMinusPiToPi(m_math_util.toRad(base_feedback.actuators[i].position));
    ...
    if (isGripperPresent())
    {
      for (int i = 0; i < base_feedback.interconnect.oneof_tool_feedback.gripper_feedback[0].motor.size(); i++)
      {
        ...
        // Arm feedback is between 0 and 100, and limits in URDF are specified in gripper_joint_limits_min[i] and
        // gripper_joint_limits_max[i] parameters
        joint_state.position[joint_state_index] = m_math_util.absolute_position_from_relative(
            base_feedback.interconnect.oneof_tool_feedback.gripper_feedback[0].motor[i].position / 100.0,
            m_gripper_joint_limits_min[i], m_gripper_joint_limits_max[i]);
        joint_state.velocity[joint_state_index] =
            base_feedback.interconnect.oneof_tool_feedback.gripper_feedback[0].motor[i].velocity;
...
void KortexArmDriver::registerSimulationHandlers()
{
  ...
  // Action services
  base_services_simulation->CreateActionHandler =
      std::bind(&KortexArmSimulation::CreateAction, m_simulator.get(), std::placeholders::_1);
  base_services_simulation->ReadActionHandler =
      std::bind(&KortexArmSimulation::ReadAction, m_simulator.get(), std::placeholders::_1);
  base_services_simulation->ReadAllActionsHandler =
      std::bind(&KortexArmSimulation::ReadAllActions, m_simulator.get(), std::placeholders::_1);
  base_services_simulation->DeleteActionHandler =
      std::bind(&KortexArmSimulation::DeleteAction, m_simulator.get(), std::placeholders::_1);
  base_services_simulation->UpdateActionHandler =
      std::bind(&KortexArmSimulation::UpdateAction, m_simulator.get(), std::placeholders::_1);
  base_services_simulation->ExecuteActionFromReferenceHandler =
      std::bind(&KortexArmSimulation::ExecuteActionFromReference, m_simulator.get(), std::placeholders::_1);
  base_services_simulation->ExecuteActionHandler =
      std::bind(&KortexArmSimulation::ExecuteAction, m_simulator.get(), std::placeholders::_1);
  base_services_simulation->StopActionHandler =
      std::bind(&KortexArmSimulation::StopAction, m_simulator.get(), std::placeholders::_1);
  // Other services
  base_services_simulation->PlayCartesianTrajectoryHandler =
      std::bind(&KortexArmSimulation::PlayCartesianTrajectory, m_simulator.get(), std::placeholders::_1);
  base_services_simulation->PlayJointTrajectoryHandler =
      std::bind(&KortexArmSimulation::PlayJointTrajectory, m_simulator.get(), std::placeholders::_1);
  base_services_simulation->SendJointSpeedsCommandHandler =
      std::bind(&KortexArmSimulation::SendJointSpeedsCommand, m_simulator.get(), std::placeholders::_1);
  base_services_simulation->SendGripperCommandHandler =
      std::bind(&KortexArmSimulation::SendGripperCommand, m_simulator.get(), std::placeholders::_1);
  base_services_simulation->StopHandler =
      std::bind(&KortexArmSimulation::Stop, m_simulator.get(), std::placeholders::_1);
  base_services_simulation->ApplyEmergencyStopHandler =
      std::bind(&KortexArmSimulation::ApplyEmergencyStop, m_simulator.get(), std::placeholders::_1);

  // Uncomment this SendTwistCommand handler to use the twist command simulation - not stable
  // base_services_simulation->SendTwistCommandHandler = std::bind(&KortexArmSimulation::SendTwistCommand,
  // m_simulator.get(), std::placeholders::_1);
