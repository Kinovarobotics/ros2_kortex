...
RobotiqGripperCommandActionServer::RobotiqGripperCommandActionServer(
    const std::string& server_name, const std::string& gripper_joint_name, double gripper_joint_limit_min,
    double gripper_joint_limit_max, ros::NodeHandle& nh, Kinova::Api::Base::BaseClient* base,
  ...
  , m_node_handle(nh)
  , m_server(nh, server_name, boost::bind(&RobotiqGripperCommandActionServer::goal_received_callback, this, _1),
             boost::bind(&RobotiqGripperCommandActionServer::preempt_received_callback, this, _1), false)
  , m_base(base)
...
void RobotiqGripperCommandActionServer::goal_received_callback(
    actionlib::ActionServer<control_msgs::GripperCommandAction>::GoalHandle new_goal_handle)
{
  ...
  double relative_position = m_math_util.relative_position_from_absolute(
      ros_gripper_command.position, m_gripper_joint_limit_min, m_gripper_joint_limit_max);
  ...
  join_polling_thread();
  m_gripper_position_polling_thread =
      std::thread(&RobotiqGripperCommandActionServer::gripper_position_polling_thread, this);
}

// Called in a separate thread when a preempt request comes in from the Action Client
void RobotiqGripperCommandActionServer::preempt_received_callback(
    actionlib::ActionServer<control_msgs::GripperCommandAction>::GoalHandle goal_handle)
{
...
void RobotiqGripperCommandActionServer::gripper_position_polling_thread()
{
  ...
  while (m_is_trajectory_running &&
         (std::chrono::duration<double>(now - m_trajectory_start_time).count() < GRIPPER_TRAJECTORY_TIME_LIMIT))
  {
    // Every 50ms
...
bool RobotiqGripperCommandActionServer::is_goal_acceptable(
    actionlib::ActionServer<control_msgs::GripperCommandAction>::GoalHandle goal_handle)
{
  ...
  double relative_position = m_math_util.relative_position_from_absolute(
      goal_handle.getGoal()->command.position, m_gripper_joint_limit_min, m_gripper_joint_limit_max);
...
bool RobotiqGripperCommandActionServer::is_goal_tolerance_respected()
{
  ...
  double goal_as_relative_position = m_math_util.relative_position_from_absolute(
      m_goal.getGoal()->command.position, m_gripper_joint_limit_min, m_gripper_joint_limit_max);
