...
CartesianTrajectoryActionServer::CartesianTrajectoryActionServer(const std::string& server_name, ros::NodeHandle& nh,
                                                                 Kinova::Api::Base::BaseClient* base,
                                                                 Kinova::Api::BaseCyclic::BaseCyclicClient* base_cyclic)
  : m_server_name(server_name)
  , m_node_handle(nh)
  , m_server(nh, server_name, boost::bind(&CartesianTrajectoryActionServer::goal_received_callback, this, _1),
             boost::bind(&CartesianTrajectoryActionServer::preempt_received_callback, this, _1), false)
  , m_base(base)
  ...
  , m_currentActionID(0)
{
  ...
  m_sub_action_notif_handle = m_base->OnNotificationActionTopic(
      std::bind(&CartesianTrajectoryActionServer::action_notif_callback, this, std::placeholders::_1),
...
void CartesianTrajectoryActionServer::goal_received_callback(
    actionlib::ActionServer<kortex_driver::FollowCartesianTrajectoryAction>::GoalHandle new_goal_handle)
{
...
void CartesianTrajectoryActionServer::preempt_received_callback(
    actionlib::ActionServer<kortex_driver::FollowCartesianTrajectoryAction>::GoalHandle goal_handle)
{
    ...
    else if (m_currentActionID == notif.handle().identifier())
    {
      switch (event)
      {
        ...
        case Kinova::Api::Base::ActionEvent::ACTION_PREPROCESS_ABORT:
          ...
          {
            ROS_DEBUG("Preprocessing has finished in the arm and goal has been rejected. Fetching the error report "
                      "from the arm...");
            ...
            for (auto error_element : report.trajectory_error_elements())
            {
              ...
              oss << "Erroneous value is " << error_element.error_value() << " but minimum permitted is "
                  << error_element.min_value() << " and maximum permitted is " << error_element.max_value()
                  << std::endl;
        ...
        case Kinova::Api::Base::ActionEvent::ACTION_ABORT:
               ...
               m_server_state == ActionServerState::TRAJECTORY_EXECUTION_PENDING))
          {
            ...
            {
              oss << "The starting point for the trajectory did not match the actual commanded cartesian pose."
                  << std::endl;
            }
...
bool CartesianTrajectoryActionServer::is_goal_acceptable(
    actionlib::ActionServer<kortex_driver::FollowCartesianTrajectoryAction>::GoalHandle goal_handle)
{
