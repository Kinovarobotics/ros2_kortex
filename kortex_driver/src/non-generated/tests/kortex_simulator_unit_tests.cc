...
protected:
  ...
  {
    ASSERT_EQ(a1.oneof_action_parameters.reach_joint_angles.size(),
              a2.oneof_action_parameters.reach_joint_angles.size());
...
TEST_F(KortexSimulatorTest, DefaultActions)
{
  ...
  for (auto a : actions)
  {
    if (a.second.name == "Retract")
      retract = true;
    else if (a.second.name == "Home")
      home = true;
    else if (a.second.name == "Zero")
      zero = true;
    else
      other = true;
  }
