...
public:
  ...
  ExecuteActionFromReference(const kortex_driver::ExecuteActionFromReference::Request& req);
  ...
  PlayCartesianTrajectory(const kortex_driver::PlayCartesianTrajectory::Request& req);
  ...
  PlayJointTrajectory(const kortex_driver::PlayJointTrajectory::Request& req);
  kortex_driver::SendJointSpeedsCommand::Response
  SendJointSpeedsCommand(const kortex_driver::SendJointSpeedsCommand::Request& req);
...
private:
  ...
  std::unique_ptr<actionlib::SimpleActionClient<control_msgs::FollowJointTrajectoryAction>>
      m_follow_joint_trajectory_action_client;
  ...
  // Helper functions
  bool IsGripperPresent() const
  {
    return !m_gripper_name.empty();
  }
  ...
  kortex_driver::KortexError FillKortexError(uint32_t code, uint32_t subCode,
